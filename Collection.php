...
class Collection implements ArrayAccess, Selectable, Arrayable, Enumerable, Jsonable, CollectionInterface
{
...
//    }
    ...
    {
        return new static(Arr::where($this->items, $p));
    }

    /**
     * Returns all the elements of this collection that are not empty.
     * The order of the elements is preserved.
     *
     * @return Collection A collection with the results of the filter operation
     */
    ...
    {
        return $this->reject(fn($v) => empty($v));
    }
    ...
    /**
     * Run a map over each of the items recursivly.
     *
     * @param  Closure $func
     * @return static
     */
    public function mapRecursive(Closure $func)
    {
        $items = $this->toArray();
        array_walk_recursive($items, function(&$item, $key) use ($func) {
            $item = $func($item, $key);
        });
